...
class VGG_A_LeakyReLU(nn.Module):
    """VGG_A model with LeakyReLU activation (negative_slope=0.1)"""
...
class VGG_A_BatchNorm(nn.Module):
    """VGG_A with Batch Normalization after each Conv layer"""
...
class VGG_A_Dropout_BN_Res(nn.Module):
    def __init__(self, inp_ch=3, num_classes=10):
        super().__init__()
...
class VGG_A_Dropout_BN_Res_Leaky(nn.Module):
    def __init__(self, inp_ch=3, num_classes=10):
        super().__init__()
        
        self.stage1 = nn.Sequential(
            ResidualBlock(inp_ch, 64, use_leaky=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        
        self.stage2 = nn.Sequential(
            ResidualBlock(64, 128, use_leaky=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        
        self.stage3 = nn.Sequential(
            ResidualBlock(128, 256, use_leaky=True),
            ResidualBlock(256, 256, use_leaky=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        
        self.stage4 = nn.Sequential(
            ResidualBlock(256, 512, use_leaky=True),
            ResidualBlock(512, 512, use_leaky=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
